...
if (scripts['postbuild:production'] !== 'npm run db:deploy') {
  throw new Error(
    'postbuild:production must deploy migrations before Hostinger launches server.js.',
  );
}
...
console.log(
  'Production lifecycle confirmed: migrations use DIRECT_URL before apps/api/dist/server.js starts.',
);

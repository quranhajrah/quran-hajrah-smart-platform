...
`build:hostinger` runs `npm ci --include=dev` before the production build. This installs TypeScript, declaration packages, Prisma tooling, and Vite for compilation without moving them into runtime dependencies. A later production-only install can still use `npm ci --omit=dev`; keep `NODE_ENV=production` for the running application.

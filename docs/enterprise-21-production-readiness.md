...
Use the official public GitHub repository, branch `main`, Node.js 20.x, repository root, `npm run build:production`, and entry file `apps/api/dist/start.js`. The entry bootstrap applies committed migrations with `npx prisma migrate deploy --schema=packages/database/prisma/schema.prisma` through `DIRECT_URL` before importing the compiled server. The production build command performs a clean install with devDependencies before compiling through `build:artifacts`; the exact operator steps are in `hostinger/`.

...
Enterprise 26.1 changes only the Executive AI writing boundary. Retrieved chunks are interpreted as themes, quantitative anchors, and status signals before audience-specific drafting. The response keeps professional prose in `answer`, places verbatim supporting quotations in a separate `supportingReferences` collection, and applies a direct-paragraph-copy guard before returning the draft. It adds CEO, Board, government-correspondence, donor-proposal, meeting-minutes, executive-report, recommendation, decision, and action-plan profiles. The Enterprise 25 gateway and all knowledge indexing, search, ranking, authorization, and confidentiality code remain unchanged; Enterprise 24 extraction is also unchanged.

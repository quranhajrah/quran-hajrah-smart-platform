...
class MemoryIdentityStore implements IdentityStore {
  ...
  listUsers = async (query: UserQuery) => {
    void query;
    return { items: this.users, total: this.users.length };
  };
  createUser = async (input: Parameters<IdentityStore['createUser']>[0]) => {
    const now = new Date();
  ...
  updateUser = async (id: string, input: Parameters<IdentityStore['updateUser']>[1]) => {
    const user = await this.findUserById(id);
  ...
  setUserRoles = async (userId: string, roleIds: string[]) => {
    ...
    if (!user) throw new Error('Not found');
    user.roles = this.roles.filter((role) => roleIds.includes(role.id));
    return user;
  };
  countActiveSuperAdmins = async () =>
    this.users.filter(
      (user) => user.isActive && user.roles.some((role) => role.name === 'super_admin'),
    ).length;
  listRoles = async () => this.roles;
  listPermissions = async () => this.permissions;
  createRole = async (input: Parameters<IdentityStore['createRole']>[0]) => {
    const role = { id: randomUUID(), ...input, isSystem: false, permissions: [] };
    this.roles.push(role);
    return role;
  };
  updateRole = async (id: string, input: Parameters<IdentityStore['updateRole']>[1]) => {
    const role = this.roles.find((item) => item.id === id);
    if (!role) throw new Error('Not found');
    Object.assign(role, input);
    return role;
  };
  setRolePermissions = async (id: string, permissionIds: string[]) => {
    const role = this.roles.find((item) => item.id === id);
    if (!role) throw new Error('Not found');
...
describe('identity and RBAC API', () => {
  ...
  beforeEach(async () => {
    ...
    store.users.push(
      ...
      {
        id: randomUUID(),
        fullName: 'مستخدم مشاهد',
        email: 'viewer@example.test',
        passwordHash: await hash(viewerPassword, 4),
        isActive: true,
        createdAt: now,
        updatedAt: now,
        roles: [store.roles[1]!],
  ...
  const login = async (email = 'admin@example.test', password = adminPassword) =>
  ...
  it('rotates a refresh token', async () => {
    const loggedIn = await request(app)
      .post('/api/auth/login')
      .send({ email: 'admin@example.test', password: adminPassword });
  ...
  it('rejects authenticated access without the required permission', async () => {
    ...
    expect(
      (await request(app).get('/api/users').set('Authorization', `Bearer ${auth.body.accessToken}`))
        .status,
    ).toBe(403);
  });
  ...
  it('creates a user, omits passwordHash, and records an audit log', async () => {
    ...
    const response = await request(app)
      ...
      .send({ fullName: 'موظف جديد', email: 'new@example.test', password: 'NewPassword123' });
    expect(response.status).toBe(201);
    expect(response.body.passwordHash).toBeUndefined();
    expect(
      store.audits.some(
        (entry) => entry.action === 'users.create' && entry.entityId === response.body.id,
      ),
    ).toBe(true);
  });
  ...
  it('prevents the final active super administrator from disabling itself', async () => {
    const auth = await login();
    const response = await request(app)
      .post(`/api/users/${store.users[0]!.id}/disable`)
      .set('Authorization', `Bearer ${auth.body.accessToken}`);
